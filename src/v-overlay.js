...
import { FocusablesHelper } from '@vaadin/vaadin-overlay/src/vaadin-focusables-helper.js';

// The same polyfills as used by mwc-dialog-base from the Material Web Components
// https://github.com/material-components/material-components-web-components/blob/master/packages/dialog/mwc-dialog-base.ts
import 'blocking-elements';
import 'wicg-inert';

const blockingElements = document.$blockingElements;

export class VOverlay extends ThemableMixin(DirMixin(PolymerElement)) {
  static get template() {
    return html`
      <style>
        :host {
  ...
  static get __attachedInstances() {
    return Array.from(document.body.children)
      .filter((el) => el instanceof VOverlay && !el.hasAttribute('closing'))
   ...
   */
  get _last() {
    return this === VOverlay.__attachedInstances.pop();
  }
  ...
  _exitModalState() {
    ...
    const instances = VOverlay.__attachedInstances;
    let el;
  ...
  bringToFront() {
    let zIndex = '';
    const frontmost = VOverlay.__attachedInstances.filter((o) => o !== this).pop();
    if (frontmost) {
...
customElements.define(VOverlay.is, VOverlay);
